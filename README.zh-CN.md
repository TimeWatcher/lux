...
Lux 当前是 alpha 软件。已经有 prerelease 二进制可用，不过从源码构建仍然是拿到当前树最快的方式：
...
`%USERPROFILE%\.lux\toolchains\<version>\luxc.exe`。仓库里提供了
`scripts/install-lux-path.ps1`，可以把 `~/.lux/bin` 加到当前用户 `PATH`。
VS Code 扩展也会直接检测 `~/.lux/bin/luxc`，所以编辑器支持不强制要求手动配置
`lux.compiler.path`。
